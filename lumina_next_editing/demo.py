...
def model_main(args, master_port, rank, request_queue, response_queue, mp_barrier):
    ...
    vae_factor = 0.18215 if train_args.vae != "sdxl" else 0.13025
    print(f"> vae factor: {vae_factor}")
    ...
    with torch.autocast("cuda", dtype):
        while True:
            ...
            try:
                ...
                x1, y1, x2, y2 = ori_image["boxes"][0]
                ori_image = ori_image["image"]

                w, h = ori_image.size
                scale = math.sqrt(w * h / train_args.image_size**2)
                w, h = round(w * scale / 16) * 16, round(h * scale / 16) * 16
                ori_image.resize((w, h), resample=Image.BOX)
                ...
                ori_latent = vae.encode(ori_image[None]).latent_dist.sample().mul_(vae_factor)[0]
                noise = torch.randn_like(ori_latent)
                edit_mask = torch.zeros_like(noise)
                edit_mask[:, :, y1 // 8 : y2 // 8, x1 // 8 : x2 // 8] = 1.0

                cap_mask = cap_mask.to(cap_feats.device)

                def inter_gt_func(t):
                    _, xt, _ = transport.path_sampler.plan(t, ori_latent, noise)
                    return xt

                x_start = inter_gt_func(t_start)
                ...
                print("> start sample")
                samples = sample_fn(x_start, model.forward_with_cfg, **model_kwargs)[-1]
                samples = samples[:1]

                samples = vae.decode(samples / vae_factor).sample
