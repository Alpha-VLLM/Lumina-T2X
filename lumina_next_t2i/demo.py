...
def model_main(args, master_port, rank, request_queue, response_queue, mp_barrier):
    ...
    with torch.autocast("cuda", dtype):
        while True:
            ...
            print("> params:", cap, resolution, num_sampling_steps, cfg_scale, solver, t_shift, seed, ntk_scaling, proportional_attn)
            try:
                # begin sampler
                ...
                model_kwargs = dict(
                    cap_feats=cap_feats, cap_mask=cap_mask, cfg_scale=cfg_scale,
                )
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    num_sampling_steps = gr.Slider(
                        minimum=1, maximum=70, value=30, interactive=True,
                ...
                with gr.Accordion("Advanced Settings for Resolution Extrapolation", open=False):
                    with gr.Row():
                        ...
                        t_shift = gr.Slider(
                            minimum=1, maximum=20, value=6, step=1, interactive=True,
                            label="Time shift"
                        )
                        cfg_scale = gr.Slider(
            ...
            with gr.Column():
                ...
                output_img = gr.Image(label="Generated image", interactive=False, format="png", value=default_img)
