...
from models.util import log_txt_as_img, default, isimage, mean_flat, default, instantiate_from_config
from .distributions.distributions import normal_kl, DiagonalGaussianDistribution
from models.autoencoder1d import VQModelInterface, IdentityFirstStage, AutoencoderKL
...
class CFM(LatentDiffusion_audio):
    ...
    def sample(self, cond, batch_size=16, timesteps=None, shape=None, x_latent=None, t_start=None, **kwargs):
        ...
        neural_ode = NeuralODE(self.ode_wrapper(cond), solver=kwargs["solver"], sensitivity="adjoint", atol=1e-4, rtol=1e-4)
    ...
    def sample_cfg(self, cond, unconditional_guidance_scale, unconditional_conditioning, batch_size=16, timesteps=None, shape=None, x_latent=None, t_start=None, **kwargs):
        ...
        neural_ode = NeuralODE(self.ode_wrapper_cfg(cond, unconditional_guidance_scale, unconditional_conditioning), solver=kwargs["solver"], sensitivity="adjoint", atol=1e-4, rtol=1e-4)
        t_span = torch.linspace(0, 1, 25 if timesteps is None else timesteps)
