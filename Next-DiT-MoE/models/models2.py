...
class TransformerBlockSandwichNorm2(nn.Module):
    def __init__(self, layer_id: int, dim: int, n_heads: int, n_kv_heads: int,
                 multiple_of: int, ffn_dim_multiplier: float, norm_eps: float,
                 qk_norm: bool, num_experts: int = 4, num_experts_per_tok: int = 2) -> None:
        """
        Initialize a TransformerBlock.
        ...
        self.feed_forward_time: nn.Module
        self.feed_forward_space: nn.Module
        if num_experts > 0:
            self.feed_forward_time = TimeMoeLayer(
                experts=[FeedForward(
                    dim=dim, hidden_dim=4 * dim, multiple_of=multiple_of,
                    ffn_dim_multiplier=ffn_dim_multiplier,) for _ in range(num_experts)],
                gate=nn.Linear(min(dim, 1024), num_experts, bias=False), 
                num_experts_per_tok=num_experts_per_tok)
            self.feed_forward_space = SpaceMoeLayer(
                experts=[FeedForward(
                    dim=dim, hidden_dim=4 * dim, multiple_of=multiple_of,
                    ffn_dim_multiplier=ffn_dim_multiplier,) for _ in range(num_experts)],
            ...
            self.feed_forward_time = FeedForward(
                dim=dim, hidden_dim=4 * dim, multiple_of=multiple_of,
                ffn_dim_multiplier=ffn_dim_multiplier,
            )
            self.feed_forward_time = FeedForward(
        ...
        self.ffn_norm_time = RMSNorm(dim, eps=norm_eps)
        self.ffn_norm_time1 = PFRMSNorm(dim, eps=norm_eps)
        self.ffn_norm_space = RMSNorm(dim, eps=norm_eps)
        self.ffn_norm_space1 = PFRMSNorm(dim, eps=norm_eps)
    ...
    def forward(
        ...
        """
        if adaln_input is not None:
            scale_msa, gate_msa, scale_mlp_time, gate_mlp_time, scale_mlp_space, gate_mlp_space = \
                self.adaLN_modulation(adaln_input).chunk(6, dim=1)
            ...
            if self.num_experts > 0:
                h = h + gate_mlp_time.unsqueeze(1).tanh() * self.ffn_norm_time(self.feed_forward_time(
                    self.ffn_norm_time1(h) * (1 + scale_mlp_time.unsqueeze(1)), time_input
                ))
                out = h + gate_mlp_space.unsqueeze(1).tanh() * self.ffn_norm_space(self.feed_forward_space(
                    self.ffn_norm_space1(h) * (1 + scale_mlp_space.unsqueeze(1)),
                ))
            else:
                h = h + gate_mlp_time.unsqueeze(1).tanh() * self.ffn_norm_time(self.feed_forward_time(
                    self.ffn_norm_time1(h) * (1 + scale_mlp_time.unsqueeze(1)), time_input
...
def DiT_Llama_600M_patch2_Both(**kwargs):
    return DiT_Llama(
