...
class Attention(nn.Module):
    ...
    def forward(
        ...
        head_num, head_dim = xk.size(-2), xk.size(-1)
        ...
        if self.proportional_attn:
            ...
            softmax_scale = math.log(seqlen, self.base_seqlen) / math.sqrt(self.head_dim)
        else:
            softmax_scale = math.sqrt(1 / self.head_dim)
        if self.base_seqlen != None and batch_query:
            q_batch_size = int(seqlen / self.base_seqlen + 0.99)
            # print(self.base_seqlen)
        else:
            q_batch_size = 1
            self.base_seqlen = seqlen
        if self.base_seqlen != None and kv_merge:
            merge_ratio = int(math.sqrt(seqlen / self.base_seqlen) + 0.5)
        else:
            merge_ratio = 1
        avg_pool = nn.AvgPool2d(kernel_size=merge_ratio, stride=merge_ratio)
        
        if self.use_flash_attn and dtype in [torch.float16, torch.bfloat16]:
            output = []
            # print(x_mask.float().mean())
            for qb_id in range(q_batch_size):
                xq_ = xq[:, self.base_seqlen * qb_id: self.base_seqlen * (qb_id + 1)]
                xk_ = xk.permute(0, 2, 3, 1).contiguous().view(bsz * head_num * head_dim, h, w)
                xk_ = avg_pool(xk_).contiguous().view(bsz, head_num, head_dim, -1).permute(0, 3, 1, 2)
                xv_ = xv.permute(0, 2, 3, 1).contiguous().view(bsz * head_num * head_dim, h, w)
                xv_ = avg_pool(xv_).contiguous().view(bsz, head_num, head_dim, -1).permute(0, 3, 1, 2)
                # print(2, xv.shape, xv_.shape)
                x_mask_ = x_mask[:, ::merge_ratio**2]
                ...
                (
                    ...
                    cu_seq_lens,
                    max_seq_lens,
                ) = self._upad_input(xq_, xk_, xv_, x_mask_, xq_.size(1))
                ...
                # print(seqlen, self.base_seqlen, query_states.shape, attn_output_unpad.shape)
                # 36864 4096 torch.Size([73728, 32, 72]) torch.Size([73728, 32, 72])
                output_ = pad_input(attn_output_unpad, indices_q, bsz, xq_.size(1))
                output.append(output_)
                # end var_len_flash_attn
                # print(0, output_.shape)
        ...
        if hasattr(self, "wk_y"):
            ...
            # print(1, output.shape, output_y.shape)
...
class NextDiT(nn.Module):
    ...
    def forward(self, x, t, cap_feats, cap_mask):
        ...
        for layer in self.layers:
            x = layer(x, mask, freqs_cis, cap_feats, cap_mask, adaln_input=adaln_input, h=h, w=w)
    ...
    #     # linear_factor = scale_factor  * (1 - timestep) + 0.7 * timestep
    #     # ntk_factor = 1.0
    #     ntk_factor = scale_factor  * (1 - timestep) + 1 * timestep
    #     linear_factor = 1.0
    ...
    def precompute_freqs_cis(
        ...
        extra_mode_list = ["none", "interpolation", "ntk_v1", "ntk_v2", "ntk_v3"]
        extra_mode = extra_mode_list[-3]
        # if timestep < 0.3:
        #     extra_mode = "interpolation"
        # else:
        #     extra_mode = "ntk_v3"
        train_seq_len = 128
        ...
        elif extra_mode == "interpolation":
            freqs = 1.0 / (theta ** (
            torch.arange(0, dim, 4)[: (dim // 4)].float().cuda() / dim
            )) 
            freqs = freqs / scale_factor
        elif extra_mode == "ntk_v1":
            theta_ = theta * scale_factor
            freqs = 1.0 / (theta_ ** (
            torch.arange(0, dim, 4)[: (dim // 4)].float().cuda() / dim
            )) 
        elif extra_mode == "ntk_v2":
            target_dim = dim * math.log(train_seq_len / 2 / math.pi) / math.log(theta)
            scale_factor = scale_factor ** (dim / target_dim)
            theta_ = theta * scale_factor
            freqs = 1.0 / (theta_ ** (
            torch.arange(0, dim, 4)[: (dim // 4)].float().cuda() / dim
            )) 
        elif extra_mode == "ntk_v3":
            freqs_inter = 1.0 / (theta ** (
                torch.arange(0, dim, 4)[: (dim // 4)].float().cuda() / dim
            )) / scale_factor

            dim_t = timestep * (dim - 1) + 1
            scale_factor = scale_factor ** (dim / dim_t)
            theta = theta * scale_factor
            freqs_ntk = 1.0 / (theta ** (
                torch.arange(0, dim, 4)[: (dim // 4)].float().cuda() / dim
            ))

            freqs = torch.max(freqs_inter, freqs_ntk)
...
def DiT_Llama_2B_GQA_patch2(**kwargs):
    return NextDiT(patch_size=2, dim=2304, n_layers=24, n_heads=32, n_kv_heads=8, **kwargs)
